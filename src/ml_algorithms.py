...
'''
Author: Rui Wang
Date: 2024-06-20 22:51:49
LastModifiedBy: Rui Wang
LastEditTime: 2024-07-16 13:50:39
Email: wang.rui@nyu.edu
FilePath: /RNAClustering/src/ml_algorithms.py
Description: 
'''
import os
import sys
...
def preprocess_data(data):
    if vertices_num == 4 or vertices_num == 5:
        indices = [i for i in np.arange(4,32)]
    else:
...
def remove_correlated_features(X, threshold=0.9):
    ...
    to_drop = [i for i in range(corr_matrix.shape[1]) if any(abs(corr_matrix[i, j]) > threshold for j in range(corr_matrix.shape[1]) if upper_tri[i, j])]
    X_uncorrelated = np.delete(X, to_drop, axis=1)
...
def save_results(results_dir, metrics, top_20_cluster_1, top_20_cluster_0, cluster_1_samples_dict, cluster_0_samples_dict):
    ...
    top_20_cluster_1_df.to_csv(os.path.join(results_dir, 'top_20_cluster_1_samples.csv'), index=False)
    ...
    top_20_cluster_0_df.to_csv(os.path.join(results_dir, 'top_20_cluster_0_samples.csv'), index=False)
    ...
    for method, samples in cluster_1_samples_dict.items():
        ...
        cluster_1_samples_df.to_csv(os.path.join(results_dir, f'cluster_1_samples_{method}.csv'), index=False)
    ...
    for method, samples in cluster_0_samples_dict.items():
        ...
        cluster_0_samples_df.to_csv(os.path.join(results_dir, f'cluster_0_samples_{method}.csv'), index=False)
